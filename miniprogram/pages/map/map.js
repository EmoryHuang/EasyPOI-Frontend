const key = "VH3BZ-BUMWU-FSUVH-G3J75-PCJIT-Z6F4J"
...
Page({
  ...
  async checkin () {
    ...
    await wx.request({
      url: app.globalData.baseUrl + '/poi/addCheckin',
      method: 'POST',
      data: {
        openid: that.data.openid,
        timestamp: timestamp,
        time: time,
        latitude: that.data.info.location.split(',')[0],
        longitude: that.data.info.location.split(',')[1],
        address: that.data.info.address,
        formatted: that.data.info.formatted,
        adInfo: that.data.info.adinfo
      },
  ...
  async getLocation (data = null) {
    ...
    let url = `https://apis.map.qq.com/ws/geocoder/v1/?key=${key}&location=${latitude},${longitude}`
    await wx.request({
      url: url,
      method: 'GET',
  ...
  async getAddress (address) {
    address = encodeURIComponent(address || '深圳市南山区深南大道10000号')
    let url = `https://apis.map.qq.com/ws/geocoder/v1/?key=${key}&address=${address}`
    await wx.request({
      url: url,
      method: 'GET',
      success: function (res) {
